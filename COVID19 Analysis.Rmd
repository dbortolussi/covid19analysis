...
US_deaths <- US_deaths %>% pivot_longer(cols=-(UID:Combined_Key),names_to="date",values_to="deaths")
...
US <- US %>% left_join(uid, by=c("Province_State", "Country_Region")) %>% select(-c(UID, FIPS)) %>% select(Province_State, Country_Region, date, cases, deaths, Population, Combined_Key)
...
US_by_state <- US_by_state %>% filter(cases>0)
...
There is no bias in this analysis.